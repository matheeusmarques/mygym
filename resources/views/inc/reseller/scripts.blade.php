...
@switch($page_name)
  ...
  @case('all-clients')
  @include('reseller.users.clients.edit')
  @include('reseller.users.clients.create')
  @include('reseller.users.clients.cdn')
  @include('reseller.users.clients.invoice')
  {{-- Table Datatable Basic Light --}}
  <script src="{{asset('plugins/table/datatable/datatables.js')}}"></script>
  <script>
  $('#zero-config').DataTable({
    ...
    order: [[ 0, "desc" ]],
    ajax:{
      url:  "{{ url('/api/masters/clientes/getdata') }}",
    },
    columns:[
      {
        data: 'id',
  ...
  $(document).ready(function() {
    $('#modal-new-user').on('show.bs.modal', function (event) {
      ...
      $.getJSON("{{url('api/masters/pacotes/clientes/getdatajson')}}", null, function(data) {
    ...
    $("#form-cdn").submit(function(event) {
      ...
      $.ajax(
        {
          type: 'POST',
          url:  "{{ url('/api/masters/clientes/cdn') }}",
      ...
      $("#form-new-user").submit(function(event) {
        ...
        $.ajax(
          {
            type: 'POST',
            url:  "{{ url('/api/masters/clientes/novo') }}",
        ...
        $("#form-generate-invoice").submit(function(event) {
          ...
          $.ajax(
            {
              type: 'POST',
              url:  "{{ url('/api/masters/clientes/gerarfatura') }}",
          ...
          $('#zero-config').DataTable({
            ...
            order: [[ 0, "desc" ]],
            ajax:{
              url:  "{{ url('/api/masters/clientes/getdataactive') }}",
            },
            columns:[
              {
                data: 'id',
              ...
              $("#form-cdn").submit(function(event) {
                ...
                $("#form-new-user").submit(function(event) {
                  ...
                  $('#zero-config').DataTable({
                    ...
                    order: [[ 0, "desc" ]],
                    ajax:{
                      url:  "{{ url('/api/masters/clientes/getdatainactive') }}",
                    },
                    columns:[
                      {
                        data: 'id',
                  ...
                  $(document).ready(function() {
                    ...
                    $("#form-generate-invoice").submit(function(event) {
                      ...
                      $.ajax(
                        {
                          type: 'POST',
                          url:  "{{ url('/api/master/clientes/gerarfatura') }}",
                          data: $('#form-generate-invoice').serialize(),
                          success: function(response)
                          {
                            $('#modal-generate-invoice').modal('toggle');
                      ...
                      $("#form-cdn").submit(function(event) {
                        ...
                        $("#form-new-user").submit(function(event) {
                          ...
                          $('#modal-edit-user').on('show.bs.modal', function (event) {
                            ...
                            console.log('teste json');
                            // event.preventDefault();
                            $.getJSON("{{url('api/master/pacotes/clientes/getdatajson')}}", null, function(data) {
                              console.log('teste json1');
                              $.each(data.packages, function(index, item) { // Iterates through a collection
                                if(item.id == package_id){
                                  $("#package_id").append( // Append an object to the inside of the select box
